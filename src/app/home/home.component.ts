...
export class HomeComponent {
  ...
  figurines: any[] = [...this.database];

  // Used for the tackBy for each section generated by the ngFor (cf input element (= the search bar) in home.componenet.html)
  figurineName(index: number, figurine: any) {
    return figurine.name;
  }

  // Select the products whose name includes what the user puts in the search bar
  onInputChange(event: Event) {
    let inputElement = event.target as HTMLInputElement;
    let inputText = inputElement.value.toLowerCase();
    let newFigurines: any[] = [...this.database];

    for (let i=0; i < newFigurines.length; i++) {
      let name:string = newFigurines[i].name.toLowerCase();
      if(!name.includes(inputText)){
        newFigurines.splice(i, 1);
        i--;
      }
    } 

    this.figurines = [...newFigurines];
  }
}
